...
void BCRexternalBWT::InsertNsymbols(uchar const * newSymb, dataTypelenSeq posSymb)
{
	...
	#if (RLO || SAP_PLUS || SAP_INVERSE || SAP_RANDOM) && PI_POS
			//update the piPos value in the vector triple
			dataTypeNSeq pos = 0;
			for(j=0; j<nExamedTexts; j++) {
				if(vectTriple[j].sap == 0 && vectTriple[j].piPos != std::numeric_limits<dataTypeNSeq>::max()) {
					pos = vectTriple[j].piPos;
				}
				else if(vectTriple[j].sap == 0 && vectTriple[j].piPos == std::numeric_limits<dataTypeNSeq>::max()) {
					vectTriple[j].piPos = pos+1;
					pos++;
				}
				else if(vectTriple[j].sap == 1) {
					pos++;
				}
			}
	#endif
...
void BCRexternalBWT::storeBWTFilePartial(uchar const * newSymb, dataTypelenSeq posSymb) {
	...
	while (j < nExamedTexts) {
		...
		while ((k< nExamedTexts) && (vectTriple[k].pileN == currentPile)) {
			...
			#if RLO || SAP_PLUS || SAP_INVERSE || SAP_RANDOM
				dataTypeNSeq start=k, end;
				dataTypeNSeq h;		
				dataTypeNSeq i = k+1;

				if(vectTriple[i-1].sap==0 && vectTriple[i].sap==1){
					start=i-1;
					while (i<nExamedTexts && vectTriple[i].sap==1) {
						i++;
					}
					end=i;
					sapPresence = true;
		  ...
		  h++;   //  I changed the number of the sequence. New iteration.
			...
			#endif
			k=h;
			//I changed the number of the sequence. New iteration.
		}
...
dataTypeNChar BCRexternalBWT::writeSymbolOnFilePartial(uchar symbol, dataTypeNChar numchar, FILE * OutFileBWT) {
 ...
 void BCRexternalBWT::makeSap(std::vector<sortElement> &v, dataTypeNSeq start, dataTypeNSeq end) {
		...
		std::cerr << "symb=";
		for (dataTypeNSeq j=0; j<end-start; j++)
			std::cerr << v[start+j].seqN  <<" ";
		std::cerr << std::endl;
		std::cerr << "sap=";
		for (dataTypeNSeq j=0; j<end-start; j++)
			std::cerr << v[start+j].sap + 48 <<" ";
		std::cerr << std::endl;
			#if BUILD_RED_SAP
			std::cerr << "newSymbSAP=";
			for (dataTypeNSeq j=0; j<end-start; j++)
...
#if SAP_PLUS
void BCRexternalBWT::makeSapPlus(std::vector<sortElement> &v, dataTypeNSeq start, dataTypeNSeq end) {
		//std::cerr << "faccio sap con prev=" << prevSymbolSap << " next=" << nextSymbolSap << "\n";

		if(prevSymbolSap == nextSymbolSap) {
			nextSymbolSap = (dataTypedimAlpha)84;
		}
	...
	bool BCRexternalBWT::cmpSapSortPlus (sortElement a,sortElement b) {
		if (newSymb[a.seqN] == newSymb[b.seqN]) {
			return (a.seqN < b.seqN);
		}
		else {
			if (newSymb[a.seqN] == nextSymbolSap) {
				return false;
			}
			else if (newSymb[b.seqN] == nextSymbolSap) {
				return true;
			}
			else if (newSymb[a.seqN] == prevSymbolSap) {
				return true;
			}
			else if (newSymb[b.seqN] == prevSymbolSap){
				return false;
			}
			else {
				return (newSymb[a.seqN] < newSymb[b.seqN]);
...
#if SAP_INVERSE
void BCRexternalBWT::makeSapInverse(std::vector<sortElement> &v, dataTypeNSeq start, dataTypeNSeq end, bool inverse) {
	//if(sapInterval) {
		
			dataTypeNChar init_posN = v[start].posN;
			//Reorder symbols in [start,end)
			if(inverse) {
				sort(v.begin()+start,v.begin()+end, cmpSapSortInverse);
			}
			else {
				sort(v.begin()+start,v.begin()+end, cmpSapSort);
			}
...
bool BCRexternalBWT::cmpSapSortInverse (sortElement a,sortElement b) {
		if (newSymb[a.seqN] == newSymb[b.seqN])
			return (b.seqN < a.seqN);
		else
...
#if SAP_RANDOM
	void BCRexternalBWT::makeSapRandom(std::vector<sortElement> &v, dataTypeNSeq start, dataTypeNSeq end) {
		dataTypeNChar init_posN = v[start].posN;
		//Reorder symbols in [start,end)
